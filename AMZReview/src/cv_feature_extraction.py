...
csv_file = r'/home/bhavika/Desktop/CS584/HW1/datapipeline/train_2.csv'

df_test = pd.read_csv('/home/bhavika/Desktop/CS584/HW1/datapipeline/train_2.csv',
...
print "find lemmatized nan" , df_test['Lemmatized'].iloc[201]
...
train_linear_comb = pd.DataFrame(lsa_train, index=train_list, columns=["Component1", "Component2", "Component3"])
test_linear_comb = pd.DataFrame(lsa_test, index=test_list, columns=["Component1", "Component2", "Component3"])

#
# b = pd.DataFrame(lsa_test, index=test_list, columns=["Component1", "Component2"])
...
# print pd.DataFrame(similarity, index=train_list, columns=test_list).head(10)
# Earlier we had test in columns and train in rows, now I'm switching it up
...
#similarity measures of train and test data split into chunks of 1500 reviews (in rows) each

# columns are test cases
# rows are train cases
...
def save_df(similarity_measures_df, chunk_size=974):
    ...
    n_files = similarity_measures_df.shape[0]/ chunk_size

    # for i in range(n_files):
    #     similarity_measures_df[start:end].to_csv('df_{}.csv'.format(i))
    #     start = end
